...
public class AimReticleScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        pInput = PlayerInputHandler.Instance;
    ...
    void LateUpdate()
    {
        ...
        if (!pInput.weaponController.inFireCone)
        {
        ...
        else if (pInput.weaponController.isOverheating)
        {
        ...
        else
        {
            ...
            for (int q = 0; q <= 2; q += 2)
            {
                for (int r = 0; r < 2; r++)
                {
                    WeaponScript thisWeapon = pInput.weaponController.weaponList[q + r];
                    ...
                    if (thisWeapon != null)
                    {
                        ...
                        else if ((pInput.weaponController.aimRange[q / 2] <= thisWeapon.weaponRange) && (pInput.weaponController.aimTag[q / 2] == "DamageObject"))
    ...
    void createTargetingBracket()
    {
        ...
        bool hittarget = Physics.Raycast(aimRay, out targetingHit, PlayerInputHandler.MAX_AIM_DISTANCE, (playerMask));
        ...
        if (hittarget)
        {
            ...
            if (pInput.isShootyTarget)
            {
                ...
                if (spawnNew)
                {
                    ...
                    currentBracket.playerMech = pInput.mechController.transform;
                    pInput.weaponController.lockedTarget = test;
                }
            }
        }
    }
